...
describe('analytic complex-I/Q synthesis', () => {
  ...
  it('retains the declared normalized DSB full-carrier AM source directly', () => {
  ...
  it('measures the unfiltered coherent AM sideband directly', () => {
    ...
    expect(measuredSideband).toBeCloseTo(unfilteredSideband, 7);
  });

  it('never turns bandwidth declaration into a hidden filter and is split-invariant', () => {
    const sampleRateHz = 1_000_000;
    ...
    for (const profile of ['am', 'fm'] as const) {
      ...
      expect(narrowBytes).toEqual(wideBytes);
      const first = synthesizeAnalyticComplexIq({
        profile,
        sampleRateHz,
        bandwidthHz: narrowBandwidthHz,
        sampleCount: split,
      });
      const second = synthesizeAnalyticComplexIq({
        profile,
        sampleRateHz,
        bandwidthHz: narrowBandwidthHz,
        sampleCount: sampleCount - split,
        startSampleIndex: split,
      });
      const joined = new Uint8Array(narrowBytes.byteLength);
      joined.set(first);
      joined.set(second, first.byteLength);
      expect(joined).toEqual(narrowBytes);
    }
  });
