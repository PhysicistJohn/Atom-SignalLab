...
function ProfileBrowser({ descriptor, catalog, active, switching, disabled, onSelect }: {
  ...
  return <div className="profile-browser">
    ...
    <article className={active ? 'active' : ''} title={descriptor.disclosure}>
      ...
      <i>{active && <b/>}</i>
      <dl><div><dt>ALLOCATION</dt><dd>{descriptor.projection.allocation.replaceAll('-', ' ').toUpperCase()}</dd></div><div><dt>TIMING</dt><dd>{descriptor.projection.timing.replaceAll('-', ' ').toUpperCase()}</dd></div><div><dt>SOURCE</dt><dd>{descriptor.source.references.map((reference) => `${reference.specification} ${reference.revision} · ${reference.clause}`).join(' + ')}</dd></div></dl>
    </article>
  </div>;
}
