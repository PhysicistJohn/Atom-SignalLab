...
/** Streaming-free SHA-256 (FIPS 180-4) over one already-assembled byte sequence. */
export function sha256HexOfBytes(...inputs: readonly (Uint8Array | string)[]): string {
  const encoder = new TextEncoder();
  const parts = inputs.map((input) => typeof input === 'string' ? encoder.encode(input) : input);
  const totalBytes = parts.reduce((sum, part) => sum + part.length, 0);
  const paddedLength = (Math.floor((totalBytes + 8) / 64) + 1) * 64;
  const padded = new Uint8Array(paddedLength);
  let offset = 0;
  for (const part of parts) { padded.set(part, offset); offset += part.length; }
...
/** Decodes charset-prevalidated base64; callers reject non-canonical input by re-encoding. */
