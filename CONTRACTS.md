...
- Complex-I/Q capability covers all 44 closed catalog profiles. `acquireIq` accepts safe-integer `centerHz` from 1 through 17,922,600,000, output `sampleRateHz` from 1,000,000 through 491,520,000, `captureBandwidthHz` from 1,000 through 491,520,000 with `captureBandwidthHz <= sampleRateHz`, and `sampleCount` from 1 through 65,536. Capture bandwidth must contain the separately declared profile `signalBandwidthHz`; it is an output setting and never an undeclared filter. `sampleFormat=cf32le` is required.
...
Historical note: contract v1 described a separately built NDJSON-over-stdio child process (`dist/bridge/atomizer-bridge.js`) with framing and process-lifecycle bounds. That implementation is deleted. V2 is a clean in-process-only boundary with no serialization, subprocess, or runtime v1 compatibility.

## Tx stream boundary

[`contracts/signal-lab-tx-stream-v1.json`](./contracts/signal-lab-tx-stream-v1.json) is a separately versioned, SignalLab-owned host-tooling surface for streaming generated I/Q toward operator transmit hardware. It is **not** the Atomizer measurement bridge: it does not change `acquireIq`, registers no Atomizer-facing capability, activates no driver, and is not an instrument or API contract. The measurement bridge v1–v3 documents and trio v4–v7 compositions remain byte-frozen.

- The stream model is chunked-pull with exactly one chunk in flight. Every output sample is a pure function of the plan and its absolute output sample coordinate, so any partition of the stream into consecutive chunks is byte-identical to the whole, and a stream resumed from any disclosed coordinate reproduces the tail exactly. Backpressure waits for sink acknowledgement; cancellation is bounded and records a `terminated` manifest; a fault stops at first failure and records a `faulted` manifest.
- Streamed bytes carry the source qualification of their profile or recipe unchanged. Chunk receipts use the same payload-kind lineage as the measurement bridge. `hardware-ready` describes deterministic output sample geometry only and never claims DAC behavior, analog reconstruction, antenna coupling, calibrated RF emission, RF conformance, product qualification, or certification.
- One-shot artifacts refuse streaming with a typed refusal that points to the matching Bluetooth long-dwell composition. Derived rates below the native artifact rate must clear the 0.95-Nyquist guard; upsampling is lossless and admitted.
- Timeline recipes are unbounded, seeded, standards-derived engineering projections over the frozen corpus generators. They carry no canonical artifact, publish their schedule model and mandated splatter/ramp/source-clean disclosures, and list exact later-promotion requirements (including a new trio version with coordinated commits).
- Custom-profile synthesis installs the plan's validated selections for the duration of each synthesized window and restores the prior state afterwards; the bytes always render the plan's disclosed configuration.
- Emission responsibility sits with the operator and their hardware. Streaming creates no standards-compliance or RF-conformance claim; `standardsCompliance` remains `not-claimed` and `rfConformance` remains `not-qualified`. Device-loop (cyclic) transmission is waveform repetition at the DAC, not a new qualified artifact.
- Scope reconciliation: trio-composition-v7 liveness item 3 (no SignalLab child process, framing loop, pipe, or shutdown acknowledgement exists) is read as governing the in-process measurement service admitted over the SignalLab-to-Atomizer edge. The Tx stream device transport introduced here is an operator-launched host-tool subprocess outside trio composition, with bounded SIGTERM-then-SIGKILL teardown and idempotent dispose. Trio v7's requirement that complex-I/Q chunking, backpressure, or long-lived sample streaming on the instrument surface needs a new instrument/API contract version remains intact for any Atomizer-facing streaming (streaming v2 plus a new trio version).
...
| Bluetooth BR/EDR + LE canonized observable projections + 2 unbounded long-dwell compositions | 4 | Standards-derived |
...
9. Complex-I/Q capability is source-declared and driver-neutral. SignalLab admits one bounded `cf32le` complex envelope for every closed profile, distinguishes native artifacts, derived transport bytes, generated output-rate bytes, and receiver-impaired bytes, and never treats RF placement metadata as artifact qualification. No measurement result claims DAC, antenna, RF calibration, packet decoding, protocol identity, broad conformance, continuous streaming, or NeptuneSDR support. (The word "current" is rescoped to "measurement" here because the separately versioned Tx stream surface now exists; the measurement-scope guarantee itself is unchanged.) The separately versioned SignalLab Tx stream surface (`contracts/signal-lab-tx-stream-v1.json`) is a host-tooling sample-stream boundary: it never alters the bounded measurement edge, creates no RF emission claim (emission responsibility sits with the operator and their hardware), and streamed bytes carry their existing qualification class unchanged. No Tx stream result claims DAC, antenna, RF calibration, packet decoding, protocol identity, broad conformance, standards compliance, RF conformance, product certification, or NeptuneSDR transmit qualification.
...
| Contract, catalog, or generator-contract-binding mismatch | Terminate the measurement admission; no retry or fallback |
| Unrepresentable exact I/Q phase or overlong receipt rational | Reject before synthesis or state change |
| Derived rate below the anti-alias guard or one-shot limit exceeded | Reject before synthesis or state change |
...
| Profile-dependent complex-I/Q synthesis and derived transport | `src/complex-iq.ts`, `src/iq-resampler.ts`, fixed profile generators |
